...
async def list_all_subjects(session: AsyncSession) -> list[dict]:
    ...
    for a, email in rows:
        subj = a.subject or a.ai_recommendation_json.get("_goal", "Không xác định") or "Không xác định"
        key = (str(a.learner_id), subj.strip().lower())
        if key not in subject_map:
...
async def rename_user_subject(
    ...
    for a in analyses:
        ...
        if effective.strip().lower() == old_subject_name.strip().lower():
    ...
    await session.execute(
        ...
        .where(
            ...
            func.lower(PersonalizedRoadmap.title) == old_subject_name.strip().lower(),
        )
...
async def delete_user_subject(
    ...
    for a in analyses:
        effective = a.subject or (a.ai_recommendation_json or {}).get("_goal", "") or ""
        if effective.strip().lower() == subject_name.strip().lower():
            ids_to_delete.append(a.id)
            if a.file_path:
                file_paths_to_delete.append(a.file_path)
    ...
    if ids_to_delete:
        ...
        await session.execute(
            ...
            .where(func.lower(PersonalizedRoadmap.title) == subject_name.strip().lower())
        )
        # Xóa ExamAnalysis
        await session.execute(
            delete(ExamAnalysis)
        ...
        # Xóa file vật lý tương ứng trên đĩa — đảm bảo uploads/ luôn đồng bộ với DB, không để
        # lại file mồ côi khi Admin xóa dữ liệu của học sinh.
        import os
        import logging
