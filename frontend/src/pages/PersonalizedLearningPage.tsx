...
function RoadmapChatbot({ subject }: { subject: string }) {
  ...
  return <div className="fixed bottom-6 right-6 z-40"><button type="button" onClick={() => setOpen((value) => !value)} className="grid size-14 place-items-center rounded-full bg-emerald-600 text-white shadow-xl shadow-emerald-900/20 transition hover:bg-emerald-700" aria-label="Hỏi chatbot về tài liệu"><MessageCircle className="size-6" /></button>{open && <div className="absolute bottom-16 right-0 w-[min(32rem,calc(100vw-2rem))] rounded-xl border border-slate-200 bg-white p-5 shadow-2xl"><h3 className="font-bold text-slate-900">{subject ? `Hỏi tài liệu ${subject}` : "Hỏi tài liệu"}</h3>{!subject ? <p className="mt-3 text-sm text-slate-600">Hãy phân tích một tài liệu học tập trước khi đặt câu hỏi.</p> : <><div className="mt-3 max-h-[min(30rem,60vh)] min-h-24 space-y-3 overflow-y-auto rounded-lg bg-slate-50 p-3">{messages.length === 0 && <p className="p-3 text-sm text-slate-500">Đặt câu hỏi để bắt đầu trao đổi với tài liệu.</p>}{messages.map((message) => <div key={message.id} className={`rounded-lg p-3 text-sm leading-6 ${message.role === "user" ? "bg-emerald-50" : "bg-white shadow-sm"}`}><div className="prose prose-sm max-w-none prose-headings:mt-3 prose-headings:mb-1 prose-p:my-1 prose-ul:my-1 prose-ol:my-1"><Markdown>{message.content}</Markdown></div></div>)}</div>{error && <p className="mt-2 text-xs text-red-600">{error}</p>}<form className="mt-3 flex gap-2" onSubmit={ask}><input className="min-w-0 flex-1 rounded-lg border border-slate-300 px-3 py-2 text-sm" value={question} onChange={(event) => setQuestion(event.target.value)} placeholder="Đặt câu hỏi..." /><button className="grid size-10 place-items-center rounded-lg bg-emerald-600 text-white disabled:opacity-50" disabled={loading} aria-label="Gửi câu hỏi"><Send className="size-4" /></button></form></>}</div>}</div>;
}
